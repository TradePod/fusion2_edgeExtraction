-- -------------------------------------------------------------
-- 
-- File Name: C:\Projects\ipcore_integration_v2\full_64_hdl\hdlsrc\gauss_sobel_rgb_64bit_full\Subsystem_ip_adapter_in_module.vhd
...
-- Source Path: Subsystem_ip/Subsystem_ip_axi4_stream_video_slave/Subsystem_ip_adapter_in/Subsystem_ip_adapter_in_module
-- Hierarchy Level: 3
-- 
-- -------------------------------------------------------------
LIBRARY IEEE;
...
ENTITY Subsystem_ip_adapter_in_module IS
...
END Subsystem_ip_adapter_in_module;


ARCHITECTURE rtl OF Subsystem_ip_adapter_in_module IS
  ...
  SIGNAL hend_reg_next                    : std_logic_vector(0 TO 1);  -- ufix1 [2]
  SIGNAL equa26_relop1                    : std_logic;
  SIGNAL vend_reg                         : std_logic;  -- ufix1
  SIGNAL vend_reg_1                       : std_logic_vector(0 TO 1);  -- ufix1 [2]
  SIGNAL vend_reg_next                    : std_logic_vector(0 TO 1);  -- ufix1 [2]
...
BEGIN
  ...
  hend <= hend_reg(1);
  hend_reg_next(0) <= hend_output_temp;
  hend_reg_next(1) <= hend_reg(0);
  ...
  BEGIN
    ...
    ELSIF clk'EVENT AND clk = '1' THEN
      IF enb = '1' THEN
        vend_reg_1(0) <= vend_reg_next(0);
        vend_reg_1(1) <= vend_reg_next(1);
      END IF;
    END IF;
  END PROCESS vend_1_process;

  vend <= vend_reg_1(1);
  vend_reg_next(0) <= vend_reg;
  vend_reg_next(1) <= vend_reg_1(0);
